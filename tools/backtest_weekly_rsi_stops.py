...
def build_picklist_for_week(universe: Dict[str, pd.DataFrame],
    ...
    Filters:
      ...
      - (optional) Close >= SMA(trend_sma)
      - (optional) Close >= previous Friday's Close (weekly-up-only)
    Returns list of (symbol, rsi) sorted desc.
    """
    rows = []
    ...
    for sym, df in universe.items():
        ...
        if not (np.isfinite(rsi) and np.isfinite(sma20)):
            continue
        # extension filter
        ext20 = abs((close_t / sma20) - 1.0)
        if ext20 > max_ext20 or rsi < rsi_min:
            continue
        # optional long-term trend filter
        if trend_sma and trend_sma > 0:
            sma_col = f'SMA{trend_sma}'
            if sma_col not in df.columns:
                df[sma_col] = df['Close'].rolling(trend_sma, min_periods=trend_sma).mean()
            smaN = df.at[t, sma_col] if pd.notna(df.at[t, sma_col]) else np.nan
            if not np.isfinite(smaN) or close_t < smaN:
                continue
        # optional weekly-up filter (current Friday >= previous Friday)
        if weekly_up_only:
            prev_week_end = t - pd.Timedelta(days=7)
            prev_candidates = dates[(dates <= prev_week_end)]
            if len(prev_candidates) == 0:
                continue
            prev_t = prev_candidates[-1]
            if close_t < df.at[prev_t, 'Close']:
                continue
...
def backtest(universe: Dict[str, pd.DataFrame],
    ...
    for week_start in mondays:
        ...
        picks = build_picklist_for_week(universe, week_start, rsi_min, max_ext20, top_per_week, trend_sma=trend_sma, weekly_up_only=weekly_up_only)
...
def main():
    ...
    ap.add_argument('--trend-sma', type=int, default=0, help='Require Close >= SMA(N) on signal day; 0 disables')
    ap.add_argument('--weekly-up-only', action='store_true', help='Require current Friday Close >= previous Friday Close')
