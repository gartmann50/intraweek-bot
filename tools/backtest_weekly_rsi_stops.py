...
Flow
----
- Signal day = last trading day before Monday (usually Friday).
- Filters at signal day:
    * RSI(14) >= rsi_min
    * |Close/SMA20 - 1| <= max_ext20
    * (optional) Close >= SMA(trend_sma)      # long-term trend filter
    * (optional) Friday Close >= prior Friday # weekly-up filter
- Pick Top-K (default 6) by RSI.
- Entry = first trading day on/after Monday at Open (with slippage).
- Stops inside week:
    * Hard stop from entry: entry * (1 - abs_stop_pct)
    * Trailing stop (if enabled): ratchets from peak intraday High:
        trail_stop = peak_high * (1 - trail_pct)
    * Active stop = max(hard_stop, trail_stop)
    * Execution model:
        - 'intraday'   (default): if Low <= stop, exit same day at min(Open, stop)
        - 'next_open': if Close <= stop, exit next day at Open
- Survivors exit on last trading day of the week at Close (with slippage).
- Equal-weight, long-only, no mid-week re-entries.

Outputs
-------
- backtests/equity_curve.csv
- backtests/trades.csv  (includes exit_reason)
...
def _infer_date_range(universe: Dict[str, pd.DataFrame]) -> Tuple[pd.Timestamp, pd.Timestamp]:
    """Infer start/end from data; start aligned to previous Monday."""
...
def backtest(universe: Dict[str, pd.DataFrame],
    ...
    for week_start in mondays:
        ...
        for sym, entry_date, entry_px in entries:
            ...
            # Days in this week [entry_date, next Monday)
            next_week_start = week_start + pd.Timedelta(days=7)
            mask = (df.index >= entry_date) & (df.index < next_week_start)
            week_days = df.index[mask]
            if len(week_days) == 0:
                continue
            ...
            for d in week_days:
                ...
                if use_stops:
                    ...
                    if stop_model == "intraday":
                        if l <= stop_lvl:
                            exit_date = d
                            exit_px   = min(o, stop_lvl) * out_mult  # gap protection
                        ...
                        if c <= stop_lvl:
                            ...
                            if nd is None:
                                ...
                                exit_date = nd
                                exit_px   = float(df.at[nd, "Open"]) * out_mult
...
def main():
    ...
    ap.add_argument("--trend-sma", type=int, default=100, help="0 disables")
